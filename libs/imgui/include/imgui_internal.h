...
IMGUI_API const char *ImParseFormatTrimDecorations(const char *format, char *buf,
                                                   size_t buf_size);
...
IMGUI_API int
ImTextStrFromUtf8(ImWchar *buf, int buf_size, const char *in_text,
...
// Helpers: Geometry
IMGUI_API ImVec2 ImBezierCalc(const ImVec2 &p1, const ImVec2 &p2, const ImVec2 &p3,
                              const ImVec2 &p4,
                              float t); // Cubic Bezier
IMGUI_API ImVec2
ImBezierClosestPoint(const ImVec2 &p1, const ImVec2 &p2, const ImVec2 &p3,
...
{
        return ImFabs((a.x * (b.y - c.y)) + (b.x * (c.y - a.y)) + (c.x * (a.y - b.y))) *
               0.5f;
}
...
struct IMGUI_API ImDrawListSharedData {
        ...
        ImVec2 ArcFastVtx
            [12 * IM_DRAWLIST_ARCFAST_TESSELLATION_MULTIPLIER]; // FIXME: Bake rounded
                                                                // corners fill/borders
                                                                // in atlas
...
enum ImGuiItemFlags_ {
        ...
        ImGuiItemFlags_ReadOnly = 1
                                  << 7, // false    // [ALPHA] Allow hovering interactions
...
enum ImGuiButtonFlagsPrivate_ {
        ...
        ImGuiButtonFlags_DontClosePopups = 1 << 13, // disable automatically closing
                                                    // parent popup on press // [UNUSED]
...
struct ImGuiSettingsHandler {
        ...
        void *(*ReadOpenFn)(ImGuiContext *ctx, ImGuiSettingsHandler *handler,
                            const char *name); // Read: Called when entering into a new
                                               // ini entry e.g. "[Window][Name]"
...
struct ImGuiContext {
        ...
        ImGuiWindow *HoveredWindow;     // Window the mouse is hovering. Will typically
                                        // catch mouse inputs.
        ImGuiWindow *HoveredRootWindow; // == HoveredWindow ? HoveredWindow->RootWindow :
                                        // NULL, merely a shortcut to avoid null test in
                                        // some situation.
        ImGuiWindow
        ...
        ImGuiID ActiveIdIsAlive; // Active widget has been seen this frame (we can't use
        ...
        ImGuiNavForward NavMoveRequestForward; // None / ForwardQueued / ForwardActive
                                               // (this is used to navigate sibling parent
...
struct IMGUI_API ImGuiWindow {
        ...
        ImVec2 ContentSize; // Size of contents/scrollable client area (calculated from
        ...
        ImDrawList *DrawList; // == &DrawListInst (for backward compatibility reason with
...
struct ImGuiTableColumn {
        ...
        ImGuiTableColumnFlags FlagsIn; // Flags as they were provided by user. See
...
struct ImGuiTable {
        ...
        float RefScale;   // Reference scale to be able to rescale columns on font/dpi
                          // changes.
        ImRect OuterRect; // Note: OuterRect.Max.y is often FLT_MAX until EndTable(),
                          // unless a height has been specified in BeginTable().
        ImRect WorkRect;
        ...
        ImGuiTableColumnIdx AutoFitSingleStretchColumn; // Index of single stretch column
                                                        // requesting auto-fit.
        ...
        ImGuiTableDrawChannelIdx
            Bg1DrawChannelCurrent; // For Selectable() and other widgets drawing
                                   // accross columns after the freezing line. Index
...
// Scrolling
IMGUI_API void
SetNextWindowScroll(const ImVec2 &scroll); // Use -1.0f on one axis to leave as-is
...
IMGUI_API ImGuiID GetIDWithSeed(const char *str_id_begin, const char *str_id_end,
                                ImGuiID seed);
...
IMGUI_API int CalcTypematicRepeatAmount(float t0, float t1, float repeat_delay,
                                        float repeat_rate);
IMGUI_API void
ActivateItem(ImGuiID id); // Remotely activate a button, checkbox, tree node etc. given
...
IMGUI_API void SetNavIDWithRectRel(ImGuiID id, int nav_layer, ImGuiID focus_scope_id,
...
IMGUI_API void RenderRectFilledRangeH(ImDrawList *draw_list, const ImRect &rect,
                                      ...
                                      float rounding);
IMGUI_API void RenderRectFilledWithHole(ImDrawList *draw_list, ImRect outer, ImRect inner,
...
IMGUI_API bool SliderBehavior(const ImRect &bb, ImGuiID id, ImGuiDataType data_type,
...
IMGUI_API T ScaleValueFromRatioT(ImGuiDataType data_type, float t, T v_min, T v_max,
                                 ...
                                 float zero_deadzone_size);
template <typename T, typename SIGNED_T, typename FLOAT_T>
IMGUI_API bool DragBehaviorT(ImGuiDataType data_type, T *v, float v_speed, T v_min,
                             T v_max, const char *format, ImGuiSliderFlags flags);
template <typename T, typename SIGNED_T, typename FLOAT_T>
IMGUI_API bool SliderBehaviorT(const ImRect &bb, ImGuiID id, ImGuiDataType data_type,
                               T *v, T v_min, T v_max, const char *format,
...
IMGUI_API bool DataTypeApplyOpFromText(const char *buf, const char *initial_value_buf,
...
IMGUI_API void DebugNodeDrawList(ImGuiWindow *window, const ImDrawList *draw_list,
                                 const char *label);
IMGUI_API void DebugNodeDrawCmdShowMeshAndBoundingBox(ImGuiWindow *window,
...
extern void ImGuiTestEngineHook_IdInfo(ImGuiContext *ctx, ImGuiDataType data_type,
                                       ImGuiID id, const void *data_id);
extern void ImGuiTestEngineHook_IdInfo(ImGuiContext *ctx, ImGuiDataType data_type,
                                       ImGuiID id, const void *data_id,

                ...
                if (line < text_end) {
                        ...
                        while (line < text_end) {
                                ...
                                if ((flags & ImGuiTextFlags_NoWidthForLargeClippedText) ==
                                    0)
                                        text_size.x = ImMax(
                                            text_size.x, CalcTextSize(line, line_end).x);
                                line = line_end + 1;
                                lines_skipped++;
                        }
                        pos.y += lines_skipped * line_height;
...
void ImGui::LabelTextV(const char *label, const char *fmt, va_list args)
{
        ...
        const char *value_text_end =
            value_text_begin +
            ImFormatStringV(g.TempBuffer, IM_ARRAYSIZE(g.TempBuffer), fmt, args);
        RenderTextClipped(value_bb.Min, value_bb.Max, value_text_begin, value_text_end,
                          NULL, ImVec2(0.0f, 0.5f));
        ...
        if (hovered) {
                if (!(flags & ImGuiButtonFlags_NoKeyModifiers) ||
                    (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt)) {
                        ...
                        // (see table above). Relies on repeat logic of IsMouseClicked()
                        // but we may as well do it ourselves if we end up exposing finer
                        // RepeatDelay/RepeatRate settings.
                        if (g.ActiveId == id && (flags & ImGuiButtonFlags_Repeat))
                                if (g.IO.MouseDownDuration[g.ActiveIdMouseButton] >
                                        0.0f &&
                ...
                if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
                        ...
                        if (g.IO.MouseDown[mouse_button]) {
                                ...
                                if ((release_in || release_anywhere) &&
                                    !g.DragDropActive) {
                                        // Report as pressed when releasing the mouse
...
bool ImGui::ButtonEx(const char *label, const ImVec2 &size_arg, ImGuiButtonFlags flags)
{
        ...
        if ((flags & ImGuiButtonFlags_AlignTextBaseLine) &&
            style.FramePadding.y <
                window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2 &size,
                          const ImVec2 &uv0, const ImVec2 &uv1, const ImVec2 &padding,
                          const ImVec4 &bg_col, const ImVec4 &tint_col)
{
        ...
        if (bg_col.w > 0.0f)
                window->DrawList->AddRectFilled(bb.Min + padding, bb.Max - padding,
                                                GetColorU32(bg_col));
        window->DrawList->AddImage(texture_id, bb.Min + padding, bb.Max - padding, uv0,
...
void ImGui::ProgressBar(float fraction, const ImVec2 &size_arg, const char *overlay)
{
        ...
        RenderRectFilledRangeH(window->DrawList, bb, GetColorU32(ImGuiCol_PlotHistogram),
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
        ...
        ImGuiContext &g = *GImGui;
        IM_ASSERT(
            ImIsPowerOfTwo(flags & (ImGuiSeparatorFlags_Horizontal |
                       ...
                       ImGuiComboFlags flags)
{
        ...
        if (ImGuiWindow *popup_window = FindWindowByName(name))
                if (popup_window->WasActive) {
                        // Always override 'AutoPosLastDirection' to not leave a chance
                        // for a past value to affect us.
...
bool ImGui::Combo(const char *label, int *current_item, const char *const items[],
...
{
        int items_count = 0;
        const char *p = items_separated_by_zeros; // FIXME-OPT: Avoid computing this, or
...
TYPE ImGui::RoundScalarWithFormatT(const char *format, ImGuiDataType data_type, TYPE v)
{
        ...
        if (fmt_start[0] != '%' || fmt_start[1] == '%') // Don't apply if the value is not
                                                        // visible in the format string
                return v;
        char v_str[64];
...
bool ImGui::DragScalarN(const char *label, ImGuiDataType data_type, void *p_data,
                        ...
                        const void *p_max, const char *format, ImGuiSliderFlags flags)
{
        ...
        for (int i = 0; i < components; i++) {
                ...
                value_changed |= DragScalar("", data_type, p_data, v_speed, p_min, p_max,
                                            format, flags);
                PopID();
                PopItemWidth();
...
bool ImGui::DragFloat2(const char *label, float v[2], float v_speed, float v_min,
                       float v_max, const char *format, ImGuiSliderFlags flags)
{
...
bool ImGui::DragFloat3(const char *label, float v[3], float v_speed, float v_min,
                       float v_max, const char *format, ImGuiSliderFlags flags)
{
...
bool ImGui::DragFloat4(const char *label, float v[4], float v_speed, float v_min,
                       float v_max, const char *format, ImGuiSliderFlags flags)
{
                            ...
                            const char *format_max, ImGuiSliderFlags flags)
{
        ...
        bool value_changed = DragScalar("##min", ImGuiDataType_Float, v_current_min,
                                        v_speed, &min_min, &min_max, format, min_flags);
        PopItemWidth();
        SameLine(0, g.Style.ItemInnerSpacing.x);
...
// NB: You likely want to specify the ImGuiSliderFlags_AlwaysClamp when using
// this.
bool ImGui::DragIntRange2(const char *label, int *v_current_min, int *v_current_max,
                        ...
                        const void *p_max, const char *format, float power)
{
        ...
        return DragScalarN(label, data_type, p_data, components, v_speed, p_min, p_max,
                           format, drag_flags);
}
                                 ...
                                 float zero_deadzone_halfsize)
{
        ...
        if (is_logarithmic) {
                                                    ...
                                                    // do this in two parts
                        {
                                ...
                                if (t_with_flip >= zero_point_snap_L &&
                                    t_with_flip <= zero_point_snap_R)
                                        result =
                                            (TYPE)0.0f; // Special case to make getting
                ...
                if (is_decimal) {
                        ...
                        // - Not doing a *1.0 multiply at the end of a range as it tends
                        // to be lossy. While absolute aiming at a large s64/u64
...
template <typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
bool ImGui::SliderBehaviorT(const ImRect &bb, ImGuiID id, ImGuiDataType data_type,
        ...
        if (g.ActiveId == id) {
                ...
                if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
                        ...
                        if (input_delta != 0.0f) {
                                ...
                                if (decimal_precision > 0) {
                                        input_delta /=
                                            100.0f; // Gamepad/keyboard tweak speeds in %
                                                    // of slider bounds
                                        ...
                                        if ((v_range >= -100.0f && v_range <= 100.0f) ||
                                            IsNavInputDown(ImGuiNavInput_TweakSlow))
                                                input_delta =
                                                    ((input_delta < 0.0f) ? -1.0f
                                                                          : +1.0f) /
                                                    (float)v_range; // Gamepad/keyboard
                                                                    // tweak speeds in
                                                                    // integer steps
                                        else
                                                input_delta /= 100.0f;
                        ...
                        if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
                                ...
                                if ((clicked_t >= 1.0f && delta > 0.0f) ||
                                    (clicked_t <= 0.0f &&
                                     delta <
                                         0.0f)) // This is to avoid applying the
                                                // saturation when already past the limits
                                {
                                        ...
                                        // Calculate what our "new" clicked_t will be, and
                                        // thus how far we actually moved the slider, and
                                        // subtract this from the accumulator
                                        TYPE v_new =
                                            ScaleValueFromRatioT<TYPE, SIGNEDTYPE,
                                                                 FLOATTYPE>(
                                        ...
                                        if (delta > 0)
                                                g.SliderCurrentAccum -= ImMin(
                                                    new_clicked_t - old_clicked_t, delta);
                                        else
                                                g.SliderCurrentAccum -= ImMax(
                                                    new_clicked_t - old_clicked_t, delta);
                                }
                         ...
                         ImGuiSliderFlags flags)
{
        ...
        // Slider behavior
        ImRect grab_bb;
        const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min,
...
bool ImGui::VSliderFloat(const char *label, const ImVec2 &size, float *v, float v_min,
                         float v_max, const char *format, ImGuiSliderFlags flags)
{
        return VSliderScalar(label, size, ImGuiDataType_Float, v, &v_min, &v_max, format,
                             flags);
}
...
bool ImGui::TempInputText(const ImRect &bb, ImGuiID id, const char *label, char *buf,
                        ...
                        ImGuiInputTextFlags flags)
{
        ...
        flags |= ImGuiInputTextFlags_NoMarkEdited; // We call MarkItemEdited() ourselves
        ...
        if (p_step != NULL) {
                ...
                if (InputText("", buf, IM_ARRAYSIZE(buf),
                        ...
                        value_changed = DataTypeApplyOpFromText(
                            buf, g.InputTextState.InitialTextA.Data, data_type, p_data,
                            format);
...
bool ImGui::InputScalarN(const char *label, ImGuiDataType data_type, void *p_data,
                        ...
                        ImGuiInputTextCallback callback, void *callback_user_data)
{
        ...
        IM_ASSERT(
            !((flags & ImGuiInputTextFlags_CallbackHistory) &&
              (flags & ImGuiInputTextFlags_Multiline))); // Can't use both together (they
        ...
        if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
                ...
                if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
                        ...
                        if (const char *clipboard = GetClipboardText()) {
                                ...
                                ImWchar *clipboard_filtered = (ImWchar *)IM_ALLOC(
        ...
        if (g.ActiveId == id) {
                ...
                if (cancel_edit) {
                        ...
                        if (!is_readonly && strcmp(buf, state->InitialTextA.Data) != 0) {
                                // Push records into the undo stack so we can CTRL+Z the
                ...
                if (apply_edit_back_to_user_buffer) {
                        ...
                        // active, should mark dirty state from the stb_textedit
                        // callbacks.
                        if (!is_readonly) {
                        ...
                        if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
                                ...
                                // (Completion/History) is that Completion needs to
                                ...
                                if (event_flag) {
                                        ...
                                        // UTF-8-positions, which can be pretty slow (an
                                        // incentive to ditch the ImWchar buffer, see
                                        // https://github.com/nothings/stb/issues/188)
                ...
                if (apply_new_text) {
                        ...
                        // apply_new_text_length' here because we have no guarantee that
                        // the size of our owned buffer matches the size of the string
                        // object held by the user, and by design we allow InputText() to
                        // be used without any storage on user's side.
                        ...
                        // IMGUI_DEBUG_LOG("InputText(\"%s\"): apply_new_text length
                        // %d\n", label, apply_new_text_length);

                        // If the underlying buffer resize was denied or not carried to
                        // the next frame, apply_new_text_length+1 may be >= buf_size.
        ...
        if (render_cursor || render_selection) {
                ...
                if (render_cursor && state->CursorFollow) {
                        ...
                        if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll)) {
                                ...
                                if (cursor_offset.x < state->ScrollX)
                                        ...
                                        state->ScrollX =
                                            IM_FLOOR(cursor_offset.x - inner_size.x +
                                                     scroll_increment_x);
                        } else {
                                state->ScrollX = 0.0f;
                        }
                        ...
                        for (const ImWchar *p = text_selected_begin;
                             p < text_selected_end;) {
                                ...
                                if (rect_pos.y < clip_rect.y) {
                                        ...
                                        // '\n', text_selected_end - p);  // FIXME-OPT:
                                        // Could use this when wchar_t are 16-bit p = p ?
                                        // p + 1 : text_selected_end;
                ...
                if (render_cursor) {
                        ...
                        // Notify OS of text input position for advanced IME (-1 x offset
                        // so that Windows IME can cover our cursor. Bit of an extra
                        // nicety.)
                        if (!is_readonly)
                                g.PlatformImePos =
...
bool ImGui::ColorEdit4(const char *label, float col[4], ImGuiColorEditFlags flags)
{
        ...
        IM_ASSERT(ImIsPowerOfTwo(
            flags & ImGuiColorEditFlags__DisplayMask)); // Check that only 1 is selected
        IM_ASSERT(ImIsPowerOfTwo(
        ...
        if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) !=
            ...
            (flags & ImGuiColorEditFlags_NoInputs) == 0) {
                ...
                for (int n = 0; n < components; n++) {
                        ...
                        // FIXME: When ImGuiColorEditFlags_HDR flag is passed HS values
                        // snap in weird ways when SV values go below 0.
                                       ...
                                       float bar_w, float alpha)
{
        ...
        ImGui::RenderArrowPointingAt(draw_list,
                                     ...
                                     IM_COL32(0, 0, 0, alpha8));
        ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x + bar_w - half_sz.x, pos.y),
                                     half_sz, ImGuiDir_Left,
                         ...
                         const float *ref_col)
{
        ...
        IM_ASSERT(ImIsPowerOfTwo(
            flags & ImGuiColorEditFlags__PickerMask)); // Check that only 1 is selected
        IM_ASSERT(ImIsPowerOfTwo(
            flags & ImGuiColorEditFlags__InputMask)); // Check that only 1 is selected
        if (!(flags & ImGuiColorEditFlags_NoOptions))
        ...
        if (value_changed_h || value_changed_sv) {
                if (flags & ImGuiColorEditFlags_InputRGB) {
                        ColorConvertHSVtoRGB(
                            H >= 1.0f ? H - 10 * 1e-6f : H, S > 0.0f ? S : 10 * 1e-6f,
                ...
                if (flags & ImGuiColorEditFlags_DisplayRGB ||
                    (flags & ImGuiColorEditFlags__DisplayMask) == 0)
                        if (ColorEdit4("##rgb", col,
                                ...
                                // InputText or DropTarget. For the later we don't want to
                                // run the hue-wrap canceling code. If you are well versed
                                // in HSV picker please provide your input! (See #2050)
        ...
        if (value_changed_fix_hue_wrap && (flags & ImGuiColorEditFlags_InputRGB)) {
                ...
                if (new_H <= 0 && H > 0) {
                        if (new_V <= 0 && V != new_V)
                                ...
                                ColorConvertHSVtoRGB(H, new_S <= 0 ? S * 0.5f : new_S,
                                                     new_V, col[0], col[1], col[2]);
                }
        }
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags)
{
        ...
        IM_ASSERT(ImIsPowerOfTwo(
            flags & ImGuiColorEditFlags__PickerMask)); // Check only 1 option is selected
        IM_ASSERT(ImIsPowerOfTwo(
            flags & ImGuiColorEditFlags__InputMask)); // Check only 1 option is selected
                       ...
                       const ImVec2 &size_arg)
{
        ...
        // extension would make explicitely right-aligned sizes not visibly match
        // other widgets.
                  ...
                  float scale_min, float scale_max, ImVec2 frame_size)
{
        ...
        if (values_count >= values_count_min) {
                ...
                if (hovered && inner_bb.Contains(g.IO.MousePos)) {
                        ...
                        const float v1 = values_getter(data, (v_idx + 1 + values_offset) %
                                                                 values_count);
                        if (plot_type == ImGuiPlotType_Lines)
                                SetTooltip("%d: %8.4g\n%d: %8.4g", v_idx, v0, v_idx + 1,
                                           v1);
                ...
                for (int n = 0; n < res_w; n++) {
                        ...
                        // Still, we should render our batch are lower level to save a bit
                        // of CPU.
...
void ImGui::PlotLines(const char *label, float (*values_getter)(void *data, int idx),
...
static int IMGUI_CDECL TabItemComparerBySection(const void *lhs, const void *rhs)
{
        ...
        const int a_section = (a->Flags & ImGuiTabItemFlags_Leading)
                                  ? 0
                                  : (a->Flags & ImGuiTabItemFlags_Trailing) ? 2 : 1;
        const int b_section = (b->Flags & ImGuiTabItemFlags_Leading)
                                  ? 0
                                  : (b->Flags & ImGuiTabItemFlags_Trailing) ? 2 : 1;
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar)
{
        ...
        for (int tab_src_n = 0; tab_src_n < tab_bar->Tabs.Size; tab_src_n++) {
                ...
                int curr_tab_section_n =
                    (tab->Flags & ImGuiTabItemFlags_Leading)
                        ? 0
                        : (tab->Flags & ImGuiTabItemFlags_Trailing) ? 2 : 1;
                if (tab_dst_n > 0) {
                        ImGuiTabItem *prev_tab = &tab_bar->Tabs[tab_dst_n - 1];
                        int prev_tab_section_n =
                            (prev_tab->Flags & ImGuiTabItemFlags_Leading)
                                ? 0
                                : (prev_tab->Flags & ImGuiTabItemFlags_Trailing) ? 2 : 1;
        ...
        ItemSize(ImVec2(tab_bar->WidthAllTabsIdeal, tab_bar->BarRect.GetHeight()),
        ...
        if (select_dir != 0)
                if (ImGuiTabItem *tab_item =
                        ...
                        while (tab_to_scroll_to == NULL) {
                                // If we are at the end of the list, still scroll to make
                                // our tab visible
                                tab_to_scroll_to =
                                ...
                                // (even if first/last item is a button, return it so we
                                // can update the scroll)
                      ...
                      ImGuiTabItemFlags flags)
{
    ...
    if (hovered && g.HoveredIdNotActiveTimer > TOOLTIP_DELAY && bb.GetWidth() < tab->ContentWidth)
    {
        ...
        bool just_closed;
        bool text_clipped;
        TabItemLabelAndCloseButton(display_draw_list, bb, flags, tab_bar->FramePadding,
                                   label, id, close_button_id, tab_contents_visible,
                              ...
                              ImGuiTabItemFlags flags, ImU32 col)
{
        ...
        draw_list->PathArcToFast(ImVec2(bb.Min.x + rounding, y1 + rounding), rounding, 6,
                                 9);
        draw_list->PathArcToFast(ImVec2(bb.Max.x - rounding, y1 + rounding), rounding, 9,
                                 12);

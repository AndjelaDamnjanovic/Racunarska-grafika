...
bool ImGui::SmallButton(const char *label)
{
        ...
        bool pressed = ButtonEx(label, ImVec2(0, 0),
...
template <typename T> static bool DataTypeClampT(T *v, const T *v_min,
                const T *v_max)
{
...
template <typename TYPE> static const char *ImAtoi(const char *src,
                TYPE *output)
{
        int negative = 0;
                                   ...
                                   TYPE v)
{
        ...
        if (fmt_start[0] != '%'
                        || fmt_start[1] == '%') // Don't apply if the value is not
                            ...
                            const char *format_max, ImGuiSliderFlags flags)
{
        ...
        float min_max = (v_min >= v_max) ? *v_current_max : ImMin(v_max,
                        *v_current_max);
        ...
        float max_min = (v_min >= v_max) ? *v_current_min : ImMax(v_min,
                        *v_current_min);
                            ...
                            ImRect *out_grab_bb)
{
        ...
        const float slider_usable_pos_min = bb.Min[axis] + grab_padding + grab_sz *
                                            0.5f;
        const float slider_usable_pos_max = bb.Max[axis] - grab_padding - grab_sz *
                                            0.5f;
                                       ...
                                       float bar_w, float alpha)
{
        ...
        ImGui::RenderArrowPointingAt(draw_list,
                                     ...
                                     IM_COL32(0, 0, 0, alpha8));
        ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x + bar_w - half_sz.x,
                                     pos.y),
...
void ImGui::PlotLines(const char *label, float (*values_getter)(void *data,
                      int idx),
...
void ImGui::PlotHistogram(const char *label, float (*values_getter)(void *data,
                          int idx),
                          void *data, int values_count, int values_offset,
                          const char *overlay_text, float scale_min, float scale_max,
                          ImVec2 graph_size)
                      ...
                      ImGuiTabItemFlags flags)
{
        ...
        if (hovered && g.HoveredIdNotActiveTimer > TOOLTIP_DELAY
                        && bb.GetWidth() < tab->ContentWidth)
        {
                ...
                TabItemBackground(display_draw_list, bb, flags,
